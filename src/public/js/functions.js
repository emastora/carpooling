...
function loadVehiclesList() {
  ...
  if (veh.brand != undefined) {
    ...
    var list_element =
      '<ons-list-item modifier="chevron" class="vehicles_list" onClick="myNavigator.pushPage(&#39;vehicle_inf.html&#39;, { animation : &#39;slide&#39; } );vehicleSelected(' +
      "'" +
      i +
      "'" +
      ');">' +
      '<ons-row>' +
      '<ons-col width="95px"><img src="' +
      pic +
      '" class="thumbnail"></ons-col>' +
      '<ons-col><div class="brand">' +
      brand +
      '&nbsp;' +
      model +
      '</div><div class="lisence_plate">' +
      licencePlate +
      '</div></ons-col>' +
      '<ons-col width="40px"></ons-col>' +
      '</ons-row>' +
...
function checkIfAccepted(obj, key, i) {
  ...
  if (!journeysAccepted[key]) {
    ...
    getUser({ email: driv, access_token: localStorage.getItem('token') }, function(d) {
      ...
      ons.notification.confirm({
        messageHTML:
          '<ons-list-item class="person" modifier="inset">' +
          '<ons-row>' +
          '<ons-col width="40px">' +
          '<img src="' +
          dr['imagePath'] +
...
function checkIfRejected(obj, key) {
  ...
  if (journeysPending[key]) {
    ...
    getUser({ email: driv, access_token: localStorage.getItem('token') }, function(d) {
      ...
      ons.notification.alert({
        messageHTML:
          '<ons-list-item class="inset" modifier="inset">' +
          '<ons-row>' +
          '<ons-col width="40px">' +
          '<img src="' +
          dr['imagePath'] +
      ...
      if (!control) {
        ...
        r.route(control.getWaypoints(), function(err, routes) {
          if (err) {
            ons.notification.alert({
              message:
                'Total distance and time of your journey could not be retrieved. Please try repositioning your destination marker.',
            ...
            ons.notification.alert({
              message:
                "Total distance of your journey can't be less than 1000m. Please try repositioning your destination marker.",
              title: 'Journey Controller Warning',
              buttonLabel: 'OK',
              animation: 'default'
            });
            ...
            ons.notification.confirm({
              ...
              messageHTML:
                "Please insert date <br /> <input type='date' id='scheduleD' required><br />And time<br><input type='time' id='scheduleT' required>", //$(function(){ $('#schedule').appendDtpicker();});
                                ...
                                ons.notification.alert({
                                  ...
                                  callback: function() {
                                    ...
                                    ons.notification.alert({
                                      ...
                                      messageHTML:
                                        '<input type="number" id="seatsAvailable" value="' +
                                        (veh[i].seats - 1) +
                                        '" min="1" max="' +
                                        (veh[i].seats - 1) +
